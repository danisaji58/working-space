...
import { Search, Users, Check, ArrowRight, Sparkles, AlertCircle } from 'lucide-react';
    ...
    async function loadSpaces() {
      ...
      try {
        const res = await getSpaces({
          tipe: selectedType === 'all' ? undefined : selectedType,
    ...
    <div className="space-y-8">
      ...
      <div className="space-y-2">
        ...
        <p className="text-xs sm:text-sm text-zinc-400 max-w-2xl">
          Temukan ruang kerja yang sesuai dengan kebutuhan fokus individual, rapat direksi, maupun kantor privat tim Anda.
        </p>
      </div>
      ...
      <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-4 p-4 rounded-2xl bg-zinc-900/60 border border-zinc-800">
        ...
        <div className="relative min-w-[240px]">
          ...
          <input
            ...
            className="w-full pl-9 pr-3.5 py-1.5 text-xs rounded-lg bg-zinc-950 border border-zinc-800 text-zinc-100 placeholder:text-zinc-500 focus:outline-none focus:border-zinc-500"
          />
        </div>
      </div>
